...
public final class MapRendering {
    ...
    public MapRendering(Map inGameMap) {
        ...
        transformation.appendScale(ClientConstants.INITIAL_SCREEN_HEIGHT / (GameConstants.MAP_RADIUS * 2), ClientConstants.INITIAL_SCREEN_HEIGHT / (GameConstants.MAP_RADIUS * 2));
...
//        context.drawImage(img, -GameConstants.MAP_RADIUS * (ratio), -GameConstants.MAP_RADIUS, 2 * ratio, 2);
        ...
        for (Mine mine : mines) {
    		...
    		context.setFill(players.get(0).getColor());
    		context.fillRect(mine.getPosition().getX() - GameConstants.MINE_RADIUS, mine.getPosition().getY() - GameConstants.MINE_RADIUS - 0.04, GameConstants.MINE_RADIUS * 2 * ownership[0], 0.02);
        }

        Image baseTexture = ImageManager.getInstance().loadImage("base.png");
        for (Base base : bases) {
        	context.setStroke(base.getOwner().getColor());
            context.drawImage(baseTexture, base.getPosition().getX() - 0.2,
                    base.getPosition().getY() - 0.2, 0.4, 0.4);
    		...
    		context.fillRect(base.getPosition().getX() - GameConstants.BASE_RADIUS, base.getPosition().getY() - GameConstants.BASE_RADIUS - 0.04, GameConstants.BASE_RADIUS * 2, 0.02);
    		context.setFill(Color.GREEN);
    		context.fillRect(base.getPosition().getX() - GameConstants.BASE_RADIUS, base.getPosition().getY() - GameConstants.BASE_RADIUS - 0.04, GameConstants.BASE_RADIUS * 2 * base.getHP() / GameConstants.INITIAL_BASE_HP, 0.02);
        }
        ...
        for (Player player : players) {
        	...
        	for(Unit unit : player.getUnits()) {
        		context.drawImage(unitTexture, unit.getPosition().getX() - 0.05,
                    unit.getPosition().getY() - 0.05, 0.1, 0.1);
        		...
        		context.setFill(Color.RED);
        		context.fillRect(unit.getPosition().getX() - GameConstants.UNIT_RADIUS, unit.getPosition().getY() - GameConstants.UNIT_RADIUS - 0.04, GameConstants.UNIT_RADIUS * 2, 0.02);
        		context.setFill(Color.GREEN);
